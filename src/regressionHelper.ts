...
function fitNegativeExponential(data: [number, number][], forceOrigin: boolean): { a: number, b: number, c: number } | null {
    const n = data.length;
    if (n < 3) return null;
...
export function getAutoParameters(data: DataPoint[], type: RegressionType, order?: number, forceOrigin?: boolean): Record<string, number> {
    ...
    if (type === 'linear') {
        ...
        if (forceOrigin) {
            ...
            coeffs.forEach((c, i) => { params[`a${i+1}`] = c; });
            return params;
        } else {
            const result = regression.polynomial(points, { order: order || 2 });
            const params: Record<string, number> = {};
            result.equation.forEach((c: number, i: number) => { params[`a${i}`] = c; });
            return params;
        }
        ...
        const validPoints = points.filter(p => p[0] > 0 && p[1] > 0).map(p => [Math.log(p[0]), Math.log(p[1])] as [number, number]);
        if (validPoints.length > 1) {
            const { m: b, c: lnA } = simpleLinearRegression(validPoints);
            return { a: Math.exp(lnA), b };
        }
    } else if (type === 'sqrt') {
        const validPoints = points.filter(p => p[0] >= 0).map(p => [Math.sqrt(p[0]), p[1]] as [number, number]);
...
export function getPredictFunction(type: RegressionType, params: Record<string, number>): ((x: number) => number) | null {
    if (type === 'linear') {
...
export function calculateRegressionDetails(data: DataPoint[], type: RegressionType, order?: number, forceOrigin?: boolean, manualParams?: Record<string, number>, manualPoints?: DataPoint[]): RegressionResult {
    if (type === 'manual') {
    ...
    if (!params || Object.keys(params).length === 0) {
        return { points: [], success: false, errorMessage: "Could not calculate parameters (check data range or validity)" };
    }

    const predict = getPredictFunction(type, params);

    if (!predict) {
        return { points: [], success: false, errorMessage: "Could not generate prediction function" };
    }
    ...
    if (fittedPoints.length === 0) {
         return { points: [], success: false, errorMessage: "Calculation resulted in invalid values (NaN/Infinity)" };
    }
...
export function calculateRegression(data: DataPoint[], type: RegressionType, order?: number, forceOrigin?: boolean, manualParams?: Record<string, number>, manualPoints?: DataPoint[]): DataPoint[] {
