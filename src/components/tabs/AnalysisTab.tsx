...
export function AnalysisTab({ series, updateSeries, editingSeriesId, setEditingSeriesId, startTransaction, commitTransaction }: AnalysisTabProps) {
    ...
    const updateRegression = (s: Series, regression: any, notify: boolean = true) => {
        ...
        if (regression.type !== 'none') {
            ...
            if (notify && regression.type !== 'manual' && regression.type !== 'spline') {
                if (result.success) {
                    addNotification('success', `Approximation found for ${s.name}: ${result.equation} (Error: ${result.error?.toExponential(2)})`);
                } else {
    ...
    const handleRegressionChange = (s: Series, type: RegressionType, order?: number, forceOrigin?: boolean) => {
        ...
        const shouldReset = type !== s.regression.type || order !== s.regression.order || forceOrigin !== s.regression.forceOrigin;
    ...
    const handleModeChange = (s: Series, mode: 'auto' | 'manual') => {
        ...
        if (mode === 'manual' && !newParams) {
            const autoParams = getAutoParameters(s.data, s.regression.type, s.regression.order, s.regression.forceOrigin);
            newParams = {};
                ...
                <div key={s.id} className="series-analysis">
                    ...
                    <div className="control-group">
                        ...
                        <CustomSelect 
                            ...
                            onChange={(val) => handleRegressionChange(s, val as RegressionType, s.regression.order, s.regression.forceOrigin)}
                            options={[
                    ...
                    {s.regression.type === 'manual' && (
                        <div className="control-group">
                            ...
                            <p className="help-text">
                                Click on plot to add points. Drag to move. Right-click to delete.
                            </p>
                        </div>
                    )}

                    {['linear', 'polynomial', 'sqrt', 'sinusoidal', 'negativeExponential'].includes(s.regression.type) && (
                        <div className="control-group">
                            <Toggle 
                                label="Force Origin (0,0)" 
                                checked={s.regression.forceOrigin || false} 
                                onChange={(checked) => handleRegressionChange(s, s.regression.type, s.regression.order, checked)}
                                onLabel="Yes"
                                offLabel="No"
                            />
                        </div>
                    ...
                    {s.regression.type === 'polynomial' && (
                        <div className="control-group">
                            <NumberInput 
                                ...
                                onChange={(val) => handleRegressionChange(s, 'polynomial', val || 2, s.regression.forceOrigin)} 
                            />
                        </div>
                    )}
                                ...
                                <div className="manual-parameters" style={{marginTop: '10px', marginBottom: '10px'}}>
                                    {Object.entries(s.regression.parameters).map(([key, param]) => (
                                        <div key={key} className="parameter-control">
                                            <label style={{fontWeight: 'bold', display: 'block', marginBottom: '5px'}}>{key}</label>
                                            <div style={{display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px'}}>
                                                <input 
                                                    type="number" 
                                                    value={param.min} 
                                                    onChange={(e) => handleParamChange(s, key, { ...param, min: parseFloat(e.currentTarget.value) })}
                                                    style={{width: '60px'}}
                                                    title="Min Value"
                                                />
                                                <input 
                                                    ...
                                                    value={param.value} 
                                                    onInput={(e) => handleParamChange(s, key, { ...param, value: parseFloat(e.currentTarget.value) })}
                                                    style={{flex: 1}}
                                                />
                                                <input 
                                                    type="number" 
                                                    value={param.max} 
                                                    onChange={(e) => handleParamChange(s, key, { ...param, max: parseFloat(e.currentTarget.value) })}
                                                    style={{width: '60px'}}
                                                    title="Max Value"
                                                />
                                            </div>
                                            <div style={{textAlign: 'center'}}>
                                                <input 
                                                    type="number" 
                                                    value={param.value} 
                                                    onChange={(e) => handleParamChange(s, key, { ...param, value: parseFloat(e.currentTarget.value) })}
                                                    style={{width: '100px'}}
                                                    title="Current Value"
                                                />
                                            </div>
                    ...
                    {s.regression.type !== 'none' && (
                        <>
                            <ColorInput 
                                ...
                                onChange={(val) => updateSeries(s.id, { regression: { ...s.regression, color: val } })} 
                            />
                            <RangeInput 
                                label="Line Width" 
                            ...
                            <div className="control-group">
                                ...
                                <CustomSelect 
                                    ...
                                    onChange={(val) => updateSeries(s.id, { regression: { ...s.regression, style: val as any } })}
                                    options={[
