...
export function PlotSettingsForm({
  ...
  commitTransaction,
  isModal = false,
}: PlotSettingsFormProps) {
  const [settingsSubTab, setSettingsSubTab] = useState<"main" | "x" | "y">("main");

  return (
    <div className={isModal ? "modal-settings-container" : "sidebar-settings-container"}>
      {!isModal && <h3 className="plot-settings-title">Plot Formatting</h3>}

      <div className="tabs" style={{ display: "flex", gap: "10px", marginBottom: "15px" }}>
        <button
          disabled={settingsSubTab === "main"}
          onClick={() => setSettingsSubTab("main")}
          style={{ flex: 1 }}
          className={settingsSubTab === "main" ? "active" : ""}
        >
          Main
        </button>
        <button
          disabled={settingsSubTab === "x"}
          onClick={() => setSettingsSubTab("x")}
          style={{ flex: 1 }}
          className={settingsSubTab === "x" ? "active" : ""}
        >
          X Axis
        </button>
        <button
          disabled={settingsSubTab === "y"}
          onClick={() => setSettingsSubTab("y")}
          style={{ flex: 1 }}
          className={settingsSubTab === "y" ? "active" : ""}
        >
          Y Axis
        </button>
      </div>
              ...
              <div className="control-group">
                <Toggle
                  ...
                  onChange={(checked) =>
                    setPlotSettings({
                      ...plotSettings,
                      hideSystemLegendOnExport: checked,
                    })
                  }
                  onLabel="Yes"
                  offLabel="No"
