...
export function DataTab({ series, setSeries, updateSeries, onAddSeries }: DataTabProps) {
    ...
    const loadManual = () => {
        ...
        if (manualFormat === 'pairs') {
            ...
            if (lines.length >= 2) {
                const xs = lines[0].trim().split(/[\s,]+/).map(parseFloat);
                const ys = lines[1].trim().split(/[\s,]+/).map(parseFloat);
                            ...
                            {renamingId === s.id ? (
                                <div style={{display: 'flex', gap: '5px', alignItems: 'center', width: '100%'}}>
                                    <input 
                                        ...
                                        autoFocus
                                    />
                                    <button onClick={saveRename} className="small-btn">OK</button>
                                    <button onClick={cancelRename} className="small-btn">Cancel</button>
                                </div>
                            ) : (
                                <>
                                    ...
                                    <div style={{ display: 'flex', gap: '8px' }}>
                                        <button onClick={() => startRenaming(s)} className="small-btn" style={{flex: 1}}>Rename</button>
                                        <button onClick={() => startEditing(s)} className="small-btn" style={{flex: 1}}>Edit Data</button>
                                        <button onClick={() => removeSeries(s.id)} className="small-btn remove-btn" style={{flex: 0}}>✕</button>
                                    </div>
                                </>
                            )}
                        </li>
            ...
            {editingId ? (
                <div className="edit-data-section">
                    ...
                    <div style={{marginTop: '10px'}}>
                        <button onClick={saveEdit} style={{marginRight: '10px'}}>Save Changes</button>
                ...
                <div className="tab-content" style={{marginTop: '10px'}}>
                    {isTauri() && activeTab === 'csv' && (
                        <div className="csv-input">
                            ...
                            <button onClick={loadCsv} disabled={!csvPath}>Load CSV</button>
                        </div>
                    )}
                    ...
                    {isTauri() && activeTab === 'excel' && (
                        <div className="excel-input">
                            ...
                            <div className="control-group">
                                ...
                                <input type="text" value={excelSheet} onInput={(e) => setExcelSheet(e.currentTarget.value)} />
                            </div>
                            <div className="control-group">
                                <label>X Range (e.g. A1:A10):</label>
                                <input type="text" value={excelXRange} onInput={(e) => setExcelXRange(e.currentTarget.value)} />
                            </div>
                            <div className="control-group">
                                <label>Y Range (e.g. B1:B10):</label>
                                <input type="text" value={excelYRange} onInput={(e) => setExcelYRange(e.currentTarget.value)} />
                            </div>
                            <button onClick={loadExcel} disabled={!excelPath}>Load Excel</button>
                        </div>
                    )}
                            ...
                            <div className="control-group">
                                ...
                                <CustomSelect 
                                    ...
                                    options={[
                                        ...
                                        { value: "lists", label: "X List then Y List (2 lines)" }
                                    ]}
                                />
                            </div>
                            <textarea 
