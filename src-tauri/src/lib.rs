...
fn read_excel(
    ...
    let x_count = if x_row_end >= x_row_start {
        x_row_end - x_row_start + 1
    } else {
        0
    };
    let y_count = if y_row_end >= y_row_start {
        y_row_end - y_row_start + 1
    } else {
        0
    };
    ...
    for i in 0..count {
        ...
        let x_val = sheet
            .get_value((r_x as u32, x_col as u32))
            .and_then(|c| match c {
                Data::Float(f) => Some(*f),
        ...
        let y_val = sheet
            .get_value((r_y as u32, y_col as u32))
            .and_then(|c| match c {
                Data::Float(f) => Some(*f),
                Data::Int(i) => Some(*i as f64),
                Data::String(s) => s.parse::<f64>().ok(),
